...
const LIQUOR_CONFIG = {
  wine: {
    ...
    // 기본 맛 균형 지표 (레드/화이트에 따라 하단 렌더링 시 동적 필터링 반영 가능 구조)
    criteria: [
      { id: 'sweetness', label: '당도 (Sweetness)', minLabel: 'Dry(드라이)', maxLabel: 'Sweet(스위트)' },
      { id: 'acidity', label: '산미 (Acidity)', minLabel: 'Low(낮은)', maxLabel: 'High(높은)' },
      { id: 'tannin', label: '타닌 (Tannin) *레드전용', minLabel: 'Low(부드러움)', maxLabel: 'High(강함)' },
      { id: 'body', label: '바디감 (Body)', minLabel: 'Light(가벼움)', maxLabel: 'Full(무거운)' },
      { id: 'finish', label: '여운 (Finish)', minLabel: 'Short(짧은)', maxLabel: 'Long(긴)' },
      { id: 'balance', label: '균형감 (Balance)', minLabel: 'Bad(안좋은)', minLabel: 'Medium', maxLabel: 'Good(좋은)' }
    ],
    // 이미지(f03e758f-2a8a-462b-b4ed-e663f9e9c2a8.jpg / ea8ea767-8e57-4d59-94d9-e8acaf1f85c3.jpg) 기준 아로마 세부 분화
    subTypes: {
      red: {
        name: '레드 와인',
        aromas: [
        ...
        excludeCriteria: [] // 모든 지표 사용 (타닌 포함)
      },
      white: {
        name: '화이트 와인 / 샴페인',
        aromas: [
          ...
          { category: '허브 / 식물 (HERBAL / VEGETAL)', items: ['허브', '풀', '민트', '피망(그린)', '아스파라거스'] },
          { category: '기타 (OTHER)', items: ['미네랄(돌, 석회질)', '꿀', '페트롤(석유향)', '스모키'] }
        ],
        excludeCriteria: ['tannin'] // 화이트 와인은 타닌 지표 제외 조치
      }
    }
  },
  whiskey: {
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 mb-6">
            ...
            {selectedLiquorType === 'wine' && (
              <div className="mb-4 flex gap-2">
                <button type="button" onClick={() => setAnalysisResult(p => ({...p, wineStyle: 'red'}))} className={`flex-1 py-1.5 text-xs font-bold rounded-xl border transition-all ${(!analysisResult?.wineStyle || analysisResult?.wineStyle === 'red') ? 'bg-rose-50 text-rose-800 border-rose-200 shadow-sm' : 'bg-gray-50 text-gray-400 border-gray-200'}`}>🍷 레드 스타일</button>
                <button type="button" onClick={() => setAnalysisResult(p => ({...p, wineStyle: 'white'}))} className={`flex-1 py-1.5 text-xs font-bold rounded-xl border transition-all ${(analysisResult?.wineStyle === 'white') ? 'bg-amber-50 text-amber-800 border-amber-200 shadow-sm' : 'bg-gray-50 text-gray-400 border-gray-200'}`}>🥂 화이트/샴페인 스타일</button>
              </div>
            )}
