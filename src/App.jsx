...
export default function TastingApp() {
  ...
  const analyzeLabel = async (base64Image) => {
    ...
    const payload = {
          // 1. 검색창에는 오직 깔끔하게 술 이름만 던집니다 (구글 검색 최적화)
          contents: [{
            role: "user",
            parts: [{
              text: `주류명: "${searchQuery}"에 대한 최신 국내 판매 가격 및 시세 정보를 구글에서 검색해줘.`
            }]
          }],
          tools: [{ "googleSearch": {} }],
          
          // 2. 무거운 제약 조건과 JSON 형식은 AI 비밀 두뇌(System)에만 주입합니다
          systemInstruction: {
            parts: [{
              text: `
                당신은 국내 주류 시세 분석 전문가입니다. 
                유저가 술 이름을 제공하면 googleSearch 도구의 실시간 웹 검색 결과(네이버 쇼핑 가격비교, 리쿼샵 공지, 블로그 리뷰 등)를 바탕으로 가장 정확한 시세를 파악해야 합니다.

                [가격 수집 지침]
                1. 소비자가 실제로 매장에서 참고할 수 있는 대략적인 가격대 범위(예: 95,000원 ~ 115,000원 부근) 형태로 유연하게 작성하세요.
                2. 국내 유통 특성상 위스키/와인은 온라인 택배 판매가 제한되므로, 단순 쇼핑몰 외에 전국의 주류점 공지 가격이나 블로그 내돈내산 후기 시세를 적극 반영하세요.
                3. 검색 결과에 명확한 가격 언급이 전혀 없다면 허위 정보를 지어내지 말고 가격 필드에 "정보없음"이라고 기입하세요.

                [출력 JSON 구조 명세 - 필수]
                반드시 다른 텍스트 없이 아래 Key들을 모두 포함한 순수한 JSON 데이터 구조 하나만 반환해야 합니다. 앞뒤 마크다운(\`\`\`json) 기호는 절대 붙이지 마세요.
                {
